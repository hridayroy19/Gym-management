...
const createBooking = async (traineeId: string, scheduleId: string) => {
    ...
    // already booked
    const existing = await Booking.findOne({ trainee: traineeId, schedule: scheduleId });
    ...
    // Check time conflict
    const traineeBookings = await Booking.find({ trainee: traineeId }).populate('schedule');
    ...
    const booking = await Booking.create({ trainee: traineeId, schedule: scheduleId });
    return booking;
};
